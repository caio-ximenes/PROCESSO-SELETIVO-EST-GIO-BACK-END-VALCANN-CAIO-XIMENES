...
for item in caminho.iterdir():
     ...
     info = item.stat()
     print(f"Nome:{item.name} | Tamanho:{transformar_byte(info.st_size)} | Data de criacao:('{datetime.fromtimestamp(info.st_ctime).strftime('%d/%m/%Y %H:%M:%S')}') | Data de modificacao:{datetime.fromtimestamp(info.st_mtime).strftime('%d/%m/%Y %H:%M:%S'),}")
     #Salva o resultado em backupsFrom.Log
     logger_listagem.info(f"Nome:{item.name}  Tamanho:{transformar_byte(info.st_size)}  Data de criacao:('{datetime.fromtimestamp(info.st_ctime).strftime('%d/%m/%Y %H:%M:%S')}')  Data de modificacao:{datetime.fromtimestamp(info.st_mtime).strftime('%d/%m/%Y %H:%M:%S'),}")
...
for item in destino_copias.iterdir():
     ...
     logger_copias.info(f"Nome:{item.name}  Tamanho:{transformar_byte(info.st_size)}  Data de criacao:('{datetime.fromtimestamp(info.st_ctime).strftime('%d/%m/%Y %H:%M:%S')}')  Data de modificação:{datetime.fromtimestamp(info.st_mtime).strftime('%d/%m/%Y %H:%M:%S'),}")
